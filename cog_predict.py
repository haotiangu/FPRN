...
try:
    ...
    print('please install cog and fprn package')
...
class Predictor(BasePredictor):
    ...
    def choose_model(self, scale, version, tile=0):
        ...
        if version == 'General - FPRNplus':
            ...
            model_path = 'weights/fprn-general-x4v3.pth'
            self.upsampler = FPRNer(
                scale=4, model_path=model_path, model=model, tile=tile, tile_pad=10, pre_pad=0, half=half)
    ...
    def predict(
        ...
        version: str = Input(
            description='FPRN version. Please see [Readme] below for more descriptions',
            choices=['General - FPRNplus', 'General - v3', 'Anime - anime6B', 'AnimeVideo - v3'],
            default='General - v3'),
