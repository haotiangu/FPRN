...
1. We first train SRNet with L1 loss from the pre-trained model ESRGAN.
...
1. Modify the content in the option file `options/train_SRNet_x4plus.yml` accordingly:
    ...
    train:
        ...
        meta_info: realesrgan/meta_info/meta_info_DF2Kmultiscale.txt  # modify to your own generate meta info txt
        io_backend:
            type: disk
    ```
...
1. Before the formal training, you may run in the `--debug` mode to see whether everything is OK. We use four GPUs for training:
    ...
    ```bash
    python FPRN/train.py -opt options/train_realesrnet_x4plus.yml --debug
    ```
...
1. After the training of Real-ESRNet, you now have the file `experiments/train_RealESRNetx4plus_1000k_B12G4_fromESRGAN/model/net_g_1000000.pth`. If you need to specify the pre-trained path to other files, modify the `pretrain_network_g` value in the option file `train_autoencoder_x4plus.yml`.
...
Only high-resolution images are required. The low-quality images are generated with the degradation process described in FPRN during training.
...
Finetune with **a single GPU**:
```bash
python realesrgan/train.py -opt options/finetune_autoencoder_x4plus_pairdata.yml --auto_resume
```
